...
class AnalysisService(DateService):
    ...
    def last_week_count(self):
        """ 最近一周总数 """
        return self.get_queryset().filter(date__range=(self._last_week, self._today)).count()
    
    def last_month_count(self):
        """ 最近一月总数 """
        return self.get_queryset().filter(date__range=(self._last_month, self._today)).count()
    
    def get_last_week_data(self):
        """ 最近一周数据 """
        return [self.get_queryset().filter(date=date).count() for date in self.get_last_week_date()]
    
    def get_last_month_data(self):
        """ 最近一月数据 """
        return [self.get_queryset().filter(date=date).count() for date in self.get_last_month_date()]
    ...
    def get_last_week_data_dict(self, format=None):
        ...
        """
        if format:  # 获取日期字符串
            return {date.strftime(format): count for date, count in zip(self.get_last_week_date(), self.get_last_week_data())}
        return dict(zip(self.get_last_week_date(), self.get_last_week_data()))
    
    def get_last_month_data_dict(self, format=None):
        """ 最近一月数据字典 """
        if format:  # 获取日期字符串
            return {date.strftime(format): count for date, count in zip(self.get_last_month_date(), self.get_last_month_data())}
        return dict(zip(self.get_last_month_date(), self.get_last_month_data()))
    
    # 日环比率
    def get_ratio(self):
        """ 日环比率 """
        if self.yesterday_count() == 0: return 0
        return round((self.today_count() - self.yesterday_count()) / self.yesterday_count() * 100, 2)
...
class OrderAnalysisService(AnalysisService):
    ...
    def yesterday_sales(self):
        ...
        return self.get_queryset().filter(
            date__range=(self._yesterday, self._today)
        ).aggregate(sales=Sum('pay_price'))['sales'] or 0
    ...
    def get_week_sales_data(self):
        ...
        return [self.get_queryset().filter(
            date=date
        ).aggregate(sales=Sum('pay_price'))['sales'] or 0 for date in self.get_last_week_date()]
    
    def get_week_sales_data_dict(self, format=None):
        """ 销售额数据字典 """
        if format:  # 获取日期字符串
            return {date.strftime(format): count for date, count in zip(self.get_last_week_date(), self.get_week_sales_data())}
    ...
    def get_month_sales_data(self):
        ...
        return [self.get_queryset().filter(
            date=date
        ).aggregate(sales=Sum('pay_price'))['sales'] or 0 for date in self.get_last_month_date()]
    
    def get_month_sales_data_dict(self, format=None):
        """ 本月销售额数据字典 """
        if format:  # 获取日期字符串
            return {date.strftime(format): count for date, count in zip(self.get_last_month_date(), self.get_month_sales_data())}
        return dict(zip(self.get_last_month_date(), self.get_month_sales_data()))
    
    def get_sales_ratio(self):
        """ 销售额环比率 """
        if self.yesterday_sales() == 0: return 0
        return round((self.today_sales() - self.yesterday_sales()) / self.yesterday_sales() * 100, 2)
...
class VisitAnalysisService(AnalysisService):
    ...
    def get_last_week_data(self):
        ...
        return [
            self._to_int_dict(
                self.get_queryset().filter(date=date).aggregate(
    ...
    def _uv_ration(self, field_name='count_uv'):
        if self.yesterday_count()[field_name] == 0: return 0
        ration = round((self.today_count()[field_name] - self.yesterday_count()[field_name]) / self.yesterday_count()[field_name] * 100, 2)
        return ration
